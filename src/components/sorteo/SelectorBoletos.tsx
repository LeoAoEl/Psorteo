  ...
  return (
    <div className="mb-6">
      ...
      <div className="grid md:grid-cols-8 grid-cols-4 lg:grid-cols-10 gap-2 mb-4">
        {boletos.map((boleto) => (
          <button
            key={boleto.id} // ID_BOLETO como identificador único
            onClick={() => boleto.estado === "libre" && toggleBoleto(boleto.id)}
            className={`p-2 text-center rounded ${getBoletoCss(
              boleto.estado,
              boletosSeleccionados.includes(boleto.id) // Comparación con ID_BOLETO
            )}`}
            disabled={boleto.estado !== "libre"}
          >
            {boleto.numero} {/* Mostrar numero_boleto */}
          </button>
        ))}
      </div>
    </div>
